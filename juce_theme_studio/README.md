...
3. Generated files are written to `.juce_theme_studio/exports/`:
   ...
   - `ThemeLayout.json` — full layout, sprite, and colour data read at runtime
   - `ThemeAssets.h` / `.cpp` — loads the exported images and slices sprite frames
   - `ThemeLookAndFeel.h` / `.cpp` — draws sprite knobs/buttons, applies palette colours
   - `GeneratedThemeComponents.h` / `.cpp` — `ThemeScreenComponent` renders a whole screen from the JSON; `applyScreenLayout()` positions your own components
   - `README-INTEGRATION.md` — CMake snippet + usage example
...
The generated C++ is functional, not a skeleton: drop `ThemeScreenComponent` into a plugin editor to render the designed theme straight from `ThemeLayout.json`, or assign sprite strips to `ThemeLookAndFeel` for sprite knobs/buttons on your existing `juce::Slider`/`juce::Button` controls. See the generated `README-INTEGRATION.md` for a CMake snippet. Existing source files are never overwritten by the exporter.
...
On project open, the scanner adds placeholder controls for detected C++ members (`gainSlider`, etc.) with JUCE class and variable mapping pre-filled. It also reads `AudioProcessorValueTreeState` `*Attachment` constructors, so a control's **Parameter ID** is filled in automatically when an attachment ties it to a parameter (e.g. `SliderAttachment(apvts, "gain", gainSlider)`). Scanned parameter IDs are offered as autocomplete on the Parameter ID field. Use **Project → Sync JUCE Mappings** or **Rescan Project** to refresh.
...
**Project → Theme Colors…** edits the palette (background, surface, primary, text,
meter colours). Values are exported into `ThemeLayout.json` and applied by the
generated `ThemeLookAndFeel`. Use **Restore Defaults** to reset.
...
3. Point to your `JuceLivePreview` binary. It renders the exported `ThemeLayout.json` —
   background, sprite frames, and labels — and reloads automatically when you re-export.
